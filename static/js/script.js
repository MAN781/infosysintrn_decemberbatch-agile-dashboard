...
// class UserSelector {
//     constructor(selectUserElement) {
//         this.selectUserElement = selectUserElement;
...
//     renderUsers(users) {
//         this.selectUserElement.innerHTML = users.map(user => `
//             <div class="user-item">
//             <input 
//                     type="checkbox" 
//                     id="user-${user.id}" 
...
//     setupEventListeners() {
//         const checkboxes = this.selectUserElement.querySelectorAll('input[type="checkbox"]');
...
//                 document.getElementById('selected-user-ids').value = 
//                     this.selectedUserIds.join(',');
//             });
//         });
//     }

//     // Method to get selected user IDs (can be used in form submission)
//     getSelectedUserIds() {
...
// // Initialize user selector when page loads
// document.addEventListener('DOMContentLoaded', () => {
//     const selectUserElement = document.getElementById('select-user');
//     const userSelector = new UserSelector(selectUserElement);

//     // Optional: If you want to get selected users on form submit
//     document.getElementById('project-form')?.addEventListener('submit', (e) => {
//         const selectedUserIds = userSelector.getSelectedUserIds();
//         console.log('Selected User IDs:', selectedUserIds);
//         // You can further process these IDs as needed
//     });
// });
...
// Function to load dropdown data for foreign keys

// Function to load users into the multi-select dropdown
...
async function loadUsersDropdown() {
    try {
        ...
        const usersSelect = document.getElementById('project-users');

        users.forEach(user => {
            const option = document.createElement('option');
            option.value = user.id; // Use the user's ID
...
async function loadDropdowns() {
    try {
        ...
        productOwners.forEach(owner => {
            const option = document.createElement('option');
            option.value = owner.id; // Use the ID from the database
...
// Function to populate a specific Scrum Master dropdown
...
// Dynamically generate sprint sections with Scrum Master dropdowns
...
function submitForm() {
    ...
    // Extracting Sprint and User Story Data (unchanged)
    jsonData.sprints = [];
    const numSprints = parseInt(document.getElementById("num-sprints").value);

    for (let i = 1; i <= numSprints; i++) {
        const sprint = {
            sprint_id: document.getElementById(`sprint-id-${i}`).value,
        ...
        userStorySections.forEach((section, index) => {
            const story = {
                ...
                user_id: section.querySelector(`#user-id-${i}-${index + 1}`).value,
                description: section.querySelector(`#story-desc-${i}-${index + 1}`).value,
                moscow: section.querySelector(`#moscow-${i}-${index + 1}`).value,
                dependency: section.querySelector(`#dependency-${i}-${index + 1}`).value,
